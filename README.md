# Earth3D · 地球启航

一个基于 Three.js 和 WebGL 的单页视觉实验，尝试重现《流浪地球 2》片尾“地球发动机点火、转向、启航”一类的电影感镜头：从舷窗式驾驶舱视角观察地球，在发动机阵列逐步点亮后完成姿态调整并驶向星空。

> 本项目是非官方的个人学习/视觉致敬作品，不隶属于电影版权方，不包含电影正片或原声。《流浪地球 2》及相关名称、设定和标识的权利归其各自权利人所有。

## 画面内容

- 舷窗式驾驶舱 HUD：姿态准星、目标锁定、镜头状态和发动机在线率读数。
- Three.js 3D 场景：地表、云层、夜景灯光、大气层、太阳、星空和空间站。
- 发动机点火序列：赤道转向发动机、行星推进发动机、尾焰与光晕会按时间轴依次出现。
- 60 秒的完整时间轴：遮蔽太阳 → 转向发动机点亮 → 地球转向 → 推进发动机点火 → 最终启航形态 → 12 种语言的告别字幕。结束后保留最终构图，气流和星空继续运动。
- “再见，太阳系”以中英、法德、西葡、意俄、阿拉伯语与印地语、韩语与印尼语六组双行字幕依次显示；每组淡入 1 秒、停留 2.6 秒、淡出 1 秒，再空白 0.4 秒。字幕与播放、暂停、重播和进度条同步。

## 运行

项目不需要构建步骤。页面默认通过相对路径加载 Three.js 的浏览器脚本；在 `file://` 模式下会自动使用 `texture-data.js` 中的内嵌纹理，绕开本地图片的跨域限制，因此可以直接双击 `index.html` 打开。若浏览器或系统策略仍限制 `file://` 下的本地资源读取，请使用本地 HTTP 静态服务器：

```powershell
cd E:\dev2026\earth3d
python -m http.server 8000
```

然后访问 <http://localhost:8000/>。

## 交互

| 操作 | 作用 |
| --- | --- |
| 拖动画面 | 手动环视地球 |
| 滚轮 | 缩放视野 |
| 双击 / 右键 / “导演视角” | 恢复自动镜头 |
| “暂停” / “重播” | 控制时间轴 |
| 空格 | 播放/暂停 |
| `R` | 从头播放 |
| `0` | 恢复自动镜头 |
| 点击进度条 / 左右方向键 | 定位动画进度 |

## 目录

```text
.
├── index.html
├── scripts/
│   ├── three.module.js       # 0.166.1 ES Module 备用版本
│   ├── three.legacy.js       # 0.166.1 浏览器直开版本
│   └── texture-data.js       # file:// 模式的内嵌纹理数据
└── img/
    ├── earth_atmos_2048.jpg
    ├── earth_clouds_1024.png
    ├── earth_lights_2048.png
    ├── earth_normal_2048.jpg
    ├── earth_specular_2048.jpg
    └── moon_1024.jpg
```

## 资源说明

Three.js 模块与行星纹理来自 three.js **0.166.1（r166）** 示例资源，相关版权和许可归原作者所有。`three.module.js` 是 ES Module 版本；`three.legacy.js` 是同版本的 CommonJS 浏览器加载副本，配合 `index.html` 内的轻量 shim 使用。由于浏览器的 `file://` 安全策略会把本地图片视为跨域资源，`texture-data.js` 保存了 6 张本地纹理的 Base64 版本，仅在直接打开 HTML 时由 `LoadingManager` 使用；HTTP 模式仍读取 `img/` 原文件。项目将依赖保存为仓库内的本地文件，以便在无 CDN 的环境中运行。
